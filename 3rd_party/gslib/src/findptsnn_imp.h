...
#define findptsnn_local_data  TOKEN_PASTE(findptsnn_local_data_,D)
...
#define findptsnn_data        TOKEN_PASTE(findptsnn_data_,D)
...
#define setupnn_aux           TOKEN_PASTE(setupnn_aux_,D)
#define findptsnn_setup       TOKEN_PASTE(PREFIXED_NAME(findptsnn_setup_),D)
#define findptsnn_free        TOKEN_PASTE(PREFIXED_NAME(findptsnn_free_ ),D)
#define findptsnn             TOKEN_PASTE(PREFIXED_NAME(findptsnn_      ),D)
#define findptsnn_eval        TOKEN_PASTE(PREFIXED_NAME(findptsnn_eval_ ),D)
                       ...
                       struct crystal *cr)
{
  ...
  #ifdef DIAGNOSTICS
  printf("findptsnn_hash(%u): local cells : %u / %u\n",cr->comm.id,nc,local_ntot);
  #endif
...
struct findptsnn_data {
  struct crystal cr;
  struct findptsnn_local_data local;
  struct hash_data hash;
};

static void setupnn_aux(
  struct findptsnn_data *const fd,
...
struct findptsnn_data *findptsnn_setup(
...
void findptsnn_free(struct findptsnn_data *fd)
{
  hash_free(&fd->hash);
  findptsnn_local_free(&fd->local);
  crystal_free(&fd->cr);
  free(fd);
}
...
//k10c      }
  ...
  /* look for other procs' points, send back */
  {
    ...
    if(src_pt.n) {
      ...
      findptsnn_local(&opt[0].code ,sizeof(struct out_pt),
  ...
  /* merge remote results with user data */
  {
    ...
    for(opt=out_pt.ptr;n;--n,++opt) {
      ...
      if (*code==CODE_NOT_FOUND || 
         ...
         (*code==CODE_BORDER && (opt->code==CODE_EXACT_BORDER || opt->code == CODE_INTERNAL || opt->dist2<=*dist2)) ||
         (*elsid != opt->elsid && *code==CODE_BORDER && opt->code==CODE_BORDER && opt->dist2<=*dist2) ||
//         (*elsid != opt->elsid && opt->dist2<=*dist2 && opt->disti>=*disti) ||
         (*elsid != opt->elsid && (*code==CODE_EXACT_BORDER || *code==CODE_INTERNAL) && 
                (opt->code==CODE_EXACT_BORDER || opt->code==CODE_INTERNAL) && opt->disti>=*disti) 
//                (opt->code==CODE_EXACT_BORDER || opt->code==CODE_INTERNAL) && opt->disti<=*disti) 
              ) 
...
void findptsnn_eval(
  ...
  const uint npt,
  const double *const in, struct findptsnn_data *const fd)
{
  ...
  /* evaluate points, send back */
  {
    ...
    for(;n;--n,++spt,++opt) opt->index=spt->index,opt->proc=spt->proc;
    spt=src.ptr, opt=outpt.ptr;
    findptsnn_local_eval(&opt->out ,sizeof(struct eval_out_pt),
...
#undef findptsnn_eval
#undef findptsnn
#undef findptsnn_free
#undef findptsnn_setup
#undef setupnn_aux
#undef eval_out_pt
#undef eval_src_pt
#undef out_pt
#undef src_pt
#undef findptsnn_data
#undef findptsnn_local_eval
#undef findptsnn_local
#undef findptsnn_local_free
#undef findptsnn_local_setup
#undef findptsnn_local_data
