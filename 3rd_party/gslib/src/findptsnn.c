...
#include "poly.h"
#include "obbox.h"
#include "findptsnn_el.h"
#include "findptsnn_local.h"
#include "gs_defs.h"
#include "comm.h"
#include "crystal.h"
...
#define ffindptsnn_setup      FORTRAN_NAME(findptsnn_setup     ,FINDPTSNN_SETUP     )
#define ffindptsnn_free       FORTRAN_NAME(findptsnn_free      ,FINDPTSNN_FREE      )
#define ffindptsnn            FORTRAN_NAME(findptsnn           ,FINDPTSNN           )
#define ffindptsnn_eval       FORTRAN_NAME(findptsnn_eval      ,FINDPTSNN_EVAL      )
#define ffindptsnn_eval_local FORTRAN_NAME(findptsnn_eval_local,FINDPTSNN_EVAL_LOCAL)
...
void ffindptsnn_setup(sint *const handle,
  ...
  const double *const distfint)
{
  ...
  h->ndim = *ndim;
  if(h->ndim==2) {
    struct findptsnn_data_2 *const fd = tmalloc(struct findptsnn_data_2,1);
    const double *elx[2];
    uint n[2], m[2];
    elx[0]=xm,elx[1]=ym;
    n[0]=*nr,n[1]=*ns;
...
void ffindptsnn_free(const sint *const handle)
{
  CHECK_HANDLE("findptsnn_free");
  if(h->ndim==2)
    PREFIXED_NAME(findptsnn_free_2)(h->data);
  else
    PREFIXED_NAME(findptsnn_free_3)(h->data);
  h->data = 0;
}

void ffindptsnn(const sint *const handle,
...
void ffindptsnn_eval(const sint *const handle,
...
{
  CHECK_HANDLE("findptsnn_eval");
  if(h->ndim==2)
    PREFIXED_NAME(findptsnn_eval_2)(
    ...
    PREFIXED_NAME(findptsnn_eval_3)(
              out_base,(* out_stride)*sizeof(double),
      (uint*)code_base,(*code_stride)*sizeof(sint  ),
      (uint*)proc_base,(*proc_stride)*sizeof(sint  ),
      (uint*)  el_base,(*  el_stride)*sizeof(sint  ),
...
void ffindptsnn_eval_local(const sint *const handle,
        double *const  out_base, const sint *const  out_stride,
  const   sint *const   el_base, const sint *const   el_stride,
  const double *const    r_base, const sint *const    r_stride,
  const sint *const npt, const double *const in)
{
  CHECK_HANDLE("findptsnn_eval_local");
  if(h->ndim==2)
    findptsnn_local_eval_2(
              out_base,(* out_stride)*sizeof(double),
      (uint*)  el_base,(*  el_stride)*sizeof(sint  ),
                r_base,(*   r_stride)*sizeof(double),
      *npt, in, &((struct findptsnn_data_2 *)h->data)->local);
  else
    findptsnn_local_eval_3(
              out_base,(* out_stride)*sizeof(double),
      (uint*)  el_base,(*  el_stride)*sizeof(sint  ),
                r_base,(*   r_stride)*sizeof(double),
      *npt, in, &((struct findptsnn_data_3 *)h->data)->local);
}
