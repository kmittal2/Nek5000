...
#define findptsnn_el_setup_2   PREFIXED_NAME(findptsnn_el_setup_2)
#define findptsnn_el_free_2    PREFIXED_NAME(findptsnn_el_free_2 )
#define findptsnn_el_2         PREFIXED_NAME(findptsnn_el_2      )
#define findptsnn_el_eval_2    PREFIXED_NAME(findptsnn_el_eval_2 )
/*
...
struct findptsnn_el_gedge_2 { const double *x[2], *dxdn[2]; };
struct findptsnn_el_gpt_2   { double x[2], jac[4], hes[4]; };

struct findptsnn_el_data_2 {
  unsigned npt_max;
  struct findptsnn_el_pt_2 *p;
  ...
  unsigned side_init;
  double *sides;
  struct findptsnn_el_gedge_2 edge[4]; /* R=-1 S; R=1 S; ... */
  struct findptsnn_el_gpt_2 pt[4];
...
void findptsnn_el_setup_2(struct findptsnn_el_data_2 *const fd,
                        const unsigned n[2],
                        const unsigned npt_max)
{
  ...
  fd->npt_max = npt_max;
  fd->p = tmalloc(struct findptsnn_el_pt_2, npt_max*2);
...
void findptsnn_el_free_2(struct findptsnn_el_data_2 *const fd)
{
  free(fd->p);
  free(fd->z[0]);
}

typedef void compute_edge_data_fun(struct findptsnn_el_data_2 *fd);

/* work[2*(nr+ns)] */
static void compute_edge_data_r(struct findptsnn_el_data_2 *fd)
{
...
/* work[4*(nr+ns)] */
static void compute_edge_data_s(struct findptsnn_el_data_2 *fd)
{
...
static const struct findptsnn_el_gedge_2 *get_edge(
  struct findptsnn_el_data_2 *fd, unsigned ei)
{
...
/* work[6*(nr+6)] */
static void compute_pt_data(struct findptsnn_el_data_2 *fd)
{
  const unsigned nr = fd->n[0], ns = fd->n[1];
...
static const struct findptsnn_el_gpt_2 *get_pt(
  struct findptsnn_el_data_2 *fd, unsigned pi)
{
...
static int reject_prior_step_q(struct findptsnn_el_pt_2 *const out,
                               const double resid[2],
                               const struct findptsnn_el_pt_2 *const p,
                               const double tol)
{
...
static void newton_area(struct findptsnn_el_pt_2 *const out,
                        const double jac[4], const double resid[2],
                        const struct findptsnn_el_pt_2 *const p, const double tol)
{
...
static void newton_edge(struct findptsnn_el_pt_2 *const out,
...
typedef void findptnn_fun(
  struct findptsnn_el_pt_2 *const out,
  struct findptsnn_el_data_2 *const fd,
  const struct findptsnn_el_pt_2 *const p, const unsigned pn, const double tol);

/* work[(6+2*(2*nr+ns))*pn] */
static void findptnn_area(
  struct findptsnn_el_pt_2 *const out,
  struct findptsnn_el_data_2 *const fd,
  const struct findptsnn_el_pt_2 *const p, const unsigned pn, const double tol)
{
  ...
  const struct findptsnn_el_pt_2 *const p, const unsigned pn, const double tol)
{
  ...
  const struct findptsnn_el_gedge_2 *const edge = get_edge(fd,ei);
  unsigned i; unsigned d;
  ...
  const struct findptsnn_el_pt_2 *const p, const unsigned pn, const double tol)
{
  ...
  const struct findptsnn_el_gpt_2 *gpt = get_pt(fd,pi);
  ...
  for(i=0;i<pn;++i) {
    ...
    /* check constraints */
    if(sr[0]<0) {
      if(sr[1]<0) goto findptnn_pt_area;
      else { de=0,dn=1; goto findptnn_pt_edge; }
    }
    else if(sr[1]<0) { de=1,dn=0; goto findptnn_pt_edge; }
...
static void seed(struct findptsnn_el_data_2 *const fd,
                 struct findptsnn_el_pt_2 *const pt, const unsigned npt)
{
  struct findptsnn_el_pt_2 *p, *const pe = pt+npt;
  const unsigned nr=fd->n[0], ns=fd->n[1];
  unsigned i,j, ii=0;
...
void findptsnn_el_2(struct findptsnn_el_data_2 *const fd, const unsigned npt,
                  const double tol)
{
  findptnn_fun *const fun[3] = 
    { &findptnn_area, &findptnn_edge, &findptnn_pt };
  struct findptsnn_el_pt_2 *const pbuf = fd->p, *const pstart = fd->p + npt;
  unsigned nconv = npt;
  unsigned step = 0;
  ...
  while(nconv && step++ < 50) {
    ...
    struct findptsnn_el_pt_2 *p, *const pe=pstart+nconv, *pout; unsigned pn;
    
#if DIAGNOSTICS_ITERATIONS>1
    { unsigned i; 
      printf("findptsnn_el_2 Newton step (%u), %u unconverged:\n ", step,nconv);
    ...
    /* group points by contsraints */
    {
      ...
      struct findptsnn_el_pt_2 *const pe = pbuf+nconv;
  ...
  { struct findptsnn_el_pt_2 *p, *const pe=pstart+npt;
    for(p=pstart;p!=pe;++p)
      pbuf[p->index]=*p, pbuf[p->index].flags&=FLAG_MASK;
  }
#if DIAGNOSTICS_ITERATIONS
  printf("findptsnn_el_2 took %u steps\n ", step);
#endif
}

void findptsnn_el_eval_2(
        double *const out_base, const unsigned out_stride,
  const double *const   r_base, const unsigned   r_stride, const unsigned pn,
  const double *const in, struct findptsnn_el_data_2 *const fd)
{
  const unsigned nr=fd->n[0],ns=fd->n[1];
