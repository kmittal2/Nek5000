...
c
     ...
     $              ,pd(lx2,ly2,lz2,lelt)
      ...
      if (istep.eq.nsteps) call outpost(ud,vd,vx,pd,t,'   ')
...
c    Global error calculation
      ...
      umx_gl = uglamax(vx,n)
      vmx_gl = uglamax(vy,n)
      pmx_gl = uglamax(pr,n2)
      uex_gl = uglamax(ue,n)
      vex_gl = uglamax(ve,n)
      pex_gl = uglamax(pe,n2)
      udx_gl = uglamax(ud,n)
      vdx_gl = uglamax(vd,n)
      pdx_gl = uglamax(pd,n2)
      end if  
      ...
      if (nid.eq.0) then
          ...
          write(6,11) istep,time,pdx,pmx,pex,v0,'  P err', session
      end if
      call nekgsync
     ...
     & 'global'
          write(6,11) istep,time,pdx_gl,pmx_gl,pex_gl,p0,'  P err',
     & 'global'
   11     format(i5,1p5e14.6,a7,2x,a7)
      ...
      if (istep.le.5) then        !  Reset velocity to eliminate 
         ...
         call copy (pr,pe,n2)      !  temporal-accuracy behavior.
      endif
