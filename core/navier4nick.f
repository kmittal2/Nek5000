...
c     Check for linear independence.
      if(alpha(m).gt.tol*norm) then !New vector is linearly independent    
         ...
         !Householder version: 2x2 Householder to annilate 
         !entries in alpha one at a time
         !More velocity iterations for some reason
c         do k = m, 2, -1  
c            beta(k) = hypot(alpha(k-1),alpha(k))     
c            beta(k) = alpha(k-1)+sign(beta(k),alpha(k-1))            
c            scale = 2.0/(beta(k)*beta(k)+alpha(k)*alpha(k))
c            c(k) = beta(k)*scale
c            s(k) = alpha(k)*scale
c            alpha(k-1) = alpha(k-1)-beta(k)*
c     $                  (alpha(k-1)*c(k)+alpha(k)*s(k))
c         enddo
c         do k = m, 2, -1
c            h = k - 1
c            do i = 1,n
c               scale = xx(i,h)*c(k) + xx(i,k)*s(k)
c               xx(i,h) = xx(i,h) - scale*beta(k)
c               xx(i,k) = xx(i,k) - scale*alpha(k)
c               scale = bb(i,h)*c(k) + bb(i,k)*s(k)
c               bb(i,h) = bb(i,h) - scale*beta(k)
c               bb(i,k) = bb(i,k) - scale*alpha(k)         
c            enddo  
c         enddo       
