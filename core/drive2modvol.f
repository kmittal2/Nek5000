...
c     then recompute base flow solution corresponding to unit forcing:
   ...
   20    continue
     ...
     $       current_flow=glsc2_univ(vx,bmg,ntot1)/domain_length  ! for X
      if (icvflow.eq.2) 
     $       current_flow=glsc2_univ(vy,bmg,ntot1)/domain_length  ! for Y
      if (icvflow.eq.3) 
     $       current_flow=glsc2_univ(vz,bmg,ntot1)/domain_length  ! for Z
...
c
      ...
      if (icalld.eq.0) then
         icalld=icalld+1
         xlmin = glmin_univ(xm1,ntot1)
         xlmax = glmax_univ(xm1,ntot1)
         ylmin = glmin_univ(ym1,ntot1)          !  for Y!
         ylmax = glmax_univ(ym1,ntot1)
         zlmin = glmin_univ(zm1,ntot1)          !  for Z!
         zlmax = glmax_univ(zm1,ntot1)
c
         ...
         if (icvflow.eq.3) domain_length = domain_length*0.5
c        THIS IS FOR THE HELIX CASE SINCE DOMAIN LENGTH IS ACTUALLY 0.5
c        TIMES THE Delta z
c
      endif
c
