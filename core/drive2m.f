...
c                - Incompressibe or Weakly compressible (div u .ne. 0).
         ...
         if (igeom.ge.2) call vol_flowk10        ! check for fixed flow rate
...
c     in userf then the true FFX is given by ffx_userf + scale.
      ...
      write(6,*) delta_flow,base_flow,'k10delbase'
...
c     forcing in X? or in Z?
      ...
      if(ifneknek) then
         if (ymin.lt.-0.9) then
          ...
          do i=1,lx1*ly1*lz1
            yv = ym1(i,1,1,e)
            wgt(i,1,1,e) = 1-0.5*(tanh(500.*(yv-0.5*(yh2+yh3)))+1);
          enddo
          enddo
         else
          do e=1,nelt
          do i=1,lx1*ly1*lz1
            yv = ym1(i,1,1,e)
            wgt(i,1,1,e) = 0.5*(tanh(500.*(yv-0.5*(yh2+yh3)))+1);
          enddo
          enddo
         endif
      endif

      if (istep.eq.2) call outpost(wgt,vy,vz,pr,t,'   ')
...
c     then recompute base flow solution corresponding to unit forcing:
   ...
   20    continue
      ...
      if (ifcomp) call compute_vol_solnk10(vxc,vyc,vzc,prc,wgt)
      if (icvflow.eq.1) current_flow=
     $                           glsc3(vx,bm1,wgt,ntot1)/domain_length  ! for X
      if (icvflow.eq.2) current_flow=
     $                           glsc3(vy,bm1,wgt,ntot1)/domain_length  ! for Y
      if (icvflow.eq.3) current_flow=
     $                           glsc3(vz,bm1,wgt,ntot1)/domain_length  ! for Z


       call neknekgsync()
       dums = current_flow
       dumr = 0.
       call get_valuesk10(dums,dumr)
       write(6,*) current_flow,dums,dumr,'k10currentflow'
       current_flow = dums+dumr

      if (iavflow.eq.1) then
         xsec = glsc2(bm1,wgt,ntot1) / domain_length
         flow_rate = param(55)*xsec
      endif
         ...
         dumr = 0.
         call get_valuesk10(dums,dumr)
        write(6,*) flow_rate,dums,dumr,'k10flowrate'
         flow_rate = dums+dumr
...
c     in userf then the true FFX is given by ffx_userf + scale.
    ...
    1    format(i8,e14.7,1p4e13.5,' volflow',1x,a1)
      ...
      write(6,*) xsec,volvm1,domain_length,current_flow,flow_rate,'k10f'
...
c-----------------------------------------------------------------------
      subroutine compute_vol_solnk10(vxc,vyc,vzc,prc,wgt)
c
c     Compute the solution to the time-dependent Stokes problem
c     with unit forcing, and find associated flow rate.
c
...
c     Compute base flow rate
c 
      if (icvflow.eq.1) base_flow =
     $                           glsc3(vxc,bm1,wgt,ntot1)/domain_length ! for X
      if (icvflow.eq.2) base_flow =
     $                           glsc3(vyc,bm1,wgt,ntot1)/domain_length ! for Y
      if (icvflow.eq.3) base_flow =
     $                           glsc3(vzc,bm1,wgt,ntot1)/domain_length ! for Z
...
c         write(6,*) idsess,nid,id,ranvar,'sending'
      enddo
      else
      do id=0,nprecv-1
       nrecv = nsize
...
c        write(6,*) idsess,nid,id,ranrec,'receiving'
      enddo
