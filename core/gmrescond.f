...
c           2-PASS GS, 1st pass:
            ...
            if (j.ne.m) call gop(hc(j+1,j),wk1,'+  ',1)
...
c           2-PASS GS, 1st pass:
            ...
            if (j.ne.m) hc(j+1,j) = sqrt(glsc2(w_gmres,w_gmres,n))
            if (j.ne.m) call gop(hc(j+1,j),wk1,'+  ',1)
...
c        if(iconv.eq.1) call dbg_write(x,nx1,ny1,nz1,nelv,'esol',3)
 ...
 9000 continue
      ...
      maxcond = glmax(condv,1)
      if (nid.eq.0) then
      write(6,9988) istep,iter,maxeig,mineig,maxcond
 9988 format(I10,' ',I8,' ',1p3e12.4,' k10condpn')
      endif
