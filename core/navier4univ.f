...
c     b <-- b - bbar
      ...
      do j=m,1,-1
         ...
         if (ifwt) then
            alpha(j)=glsc3_univ(xx(1,j),w,b,n)
c            alpha(j)=glsc3(xx(1,j),w,b,n)
         else
            alpha(j)=glsc2_univ(xx(1,j),b,n)
c            alpha(j)=glsc2(xx(1,j),b,n)
         endif
...
c-----------------------------------------------------------------------
      ...
      if (      ifwt) alpha = glsc3_univ(xx(1,m),w,bb(1,m),n)
      if (.not. ifwt) alpha = glsc2_univ(xx(1,m),bb(1,m),n)
c      if (      ifwt) alpha = glsc3(xx(1,m),w,bb(1,m),n)
c      if (.not. ifwt) alpha = glsc2(xx(1,m),bb(1,m),n)
      ...
      do k=m-1,1,-1  ! Reorthogonalize, starting with latest solution
         ...
         if (      ifwt) normk = glsc3_univ(xx(1,k),w,bb(1,k),n)
c        if (      ifwt) normk = glsc3(xx(1,k),w,bb(1,k),n)
         if (.not. ifwt) normk = glsc2_univ(xx(1,k),bb(1,k),n)
c         if (.not. ifwt) normk = glsc2(xx(1,k),bb(1,k),n)
         normk=sqrt(normk)
     ...
     $                       +     vlsc2(bb(1,j),xx(1,k),n))
               ...
               scale = -glsum_univ(alpha,1)
c               scale = -glsum(alpha,1)
               call add2s2(xx(1,k),xx(1,j),scale,n)
               call add2s2(bb(1,k),bb(1,j),scale,n)
            endif
         enddo
         if (      ifwt) normp = glsc3_univ(xx(1,k),w,bb(1,k),n)
         if (.not. ifwt) normp = glsc2_univ(xx(1,k),bb(1,k),n)
c         if (      ifwt) normp = glsc3(xx(1,k),w,bb(1,k),n)
c         if (.not. ifwt) normp = glsc2(xx(1,k),bb(1,k),n)
